...
export class GemstoneService {
  ...
  async searchGemstones(type?: string, minPrice?: number, maxPrice?: number, origin?: string, certificationStatus?: boolean) {
    const where: any = {};
    ...
    if (certificationStatus !== undefined) where.certificationStatus = certificationStatus;
